...
library(ggplot2)
library(sp)
setwd("D:/Data/Semi_JWTC202007/RI_NoRI/正在用到的文件/5.8draw/Results/NoRI/")
...
for (i in 1:length(year_RI)) {
  ...
  data_In<-read.csv(paste0(result_path,year_RI[i],"_NoRI_InofTC.csv"))#row header is not included
  data_1_1<-read.csv(paste0(result_path,year_RI[i],"_NoRI_1_1.csv"))
  data_1_0<-read.csv(paste0(result_path,year_RI[i],"_NoRI_1_0.csv"))
  ...
  write_path<-"D:/Data/Semi_JWTC202007/RI_NoRI/正在用到的文件/5.99all_data/l_s/NoRI/"
  write.csv(data_In3,paste0(write_path,year_RI[i],"_NoRI_data_In.csv"),quote=F)
}
