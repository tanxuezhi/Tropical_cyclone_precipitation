...
ff<-read.csv("p95_trend_record.csv",stringsAsFactors=F)
ff$region[which(is.na(ff$region)==TRUE)]<-"NA"

ww<-1.9
cate4<-c("NoRI_land","RI_land","NoRI_ocean","RI_ocean")
limit_y<-c(min(ff$index),max(ff$index))
bb<-c(min(ff$index),0.00,max(ff$index)/2,max(ff$index))
lla<-round(c(min(ff$index),0.00,max(ff$index)/2,max(ff$index)),2)
...
for(i in 1:length(cate4)){
  ...
  w1<-ggplot(dd,aes(x=id,y=index,fill=region))+
    ...
    mytheme1
  print(w1)
  ggsave(filename=paste0("newpictures/P6/p95/",cate4[i],".png"),w1,width=ww,height=ww*0.89,bg = "transparent")
  ggsave(filename=paste0("newpictures/P6/p95/",cate4[i],".pdf"),w1,width=ww,height=ww*0.89,bg = "transparent")
