...
for(i in 1:length(cate4)){
  ...
  w1<-ggplot(dd,aes(x=id,y=index,fill=region))+
    ...
    scale_y_continuous(limits=limit_y,expand=c(0,0))+
    mytheme1
  print(w1)
  ggsave(filename=paste0("newpictures/P1/",cate4[i],".png"),w1,width=1.74,height=1.7,bg = "transparent")
  ggsave(filename=paste0("newpictures/P1/",cate4[i],".pdf"),w1,width=1.74,height=1.7,bg = "transparent")
