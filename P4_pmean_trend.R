...
ff<-read.csv("pmean_trend_record.csv",stringsAsFactors=F)
...
for(i in 1:length(cate4)){
  ...
  w1<-ggplot(dd,aes(x=id,y=index,fill=region))+
    ...
    mytheme1
  print(w1)
  ggsave(filename=paste0("newpictures/P4/",cate4[i],".png"),w1,width=ww,height=ww*0.89,bg = "transparent")
  ggsave(filename=paste0("newpictures/P4/",cate4[i],".pdf"),w1,width=ww,height=ww*0.89,bg = "transparent")
