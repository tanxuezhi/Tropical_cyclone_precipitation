...
for (i in 1:length(year)){
  print(year[i])
  InofTC_all <- read.table(paste0("Tracks/NoRI/",year[i],"_NoRI.csv"),header=TRUE,sep=",")
  
  # create folder to record data
  Result_path <- paste0("Results/NoRI/",year[i],"/")
      ...
      for (PP in 1:length(filenames_P)){
        ...
        if (oneyear_TC == year_P & onemon_TC == month_P & oneday_TC == day_P){
          ...
          write.csv(pre,paste0(pre_path,"line",k,"_",onemon_TC,"_",InofTC$day_TC[k],"_",
                               ...
                               InofTC$area[k],"_",
                               InofTC$rank_lat_TC[k],"_",InofTC$rank_lon_TC[k],"_NoRI.csv"))
  ...
  write.csv(RI_1_1,paste0(Result_path,year[i],'_','NoRI_1_1.csv'),quote=F)
  write.csv(RI_1_0,paste0(Result_path,year[i],'_','NoRI_1_0.csv'),quote=F)
  write.csv(InofTC,paste0(Result_path,year[i],'_','NoRI_InofTC.csv'),quote=F)
  write.csv(InofTC_more,paste0(Result_path,year[i],'_','NoRI_InofTC_more.csv'),quote=F)
}
